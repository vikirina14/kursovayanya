...
if (seasonPrevBtn) {
  seasonPrevBtn.addEventListener('click', () => {
    seasonStartIndex = (seasonStartIndex - 1 + seasonsData.length) % seasonsData.length;
    renderSeasonList();
  });
}
...
// Показать локацию по индексу
function showLocation(index) {
  locationIndex = (index + locationOrder.length) % locationOrder.length;

  locationOrder.forEach((id, i) => {
    const block = document.getElementById(id);
    if (!block) return;

    // Скрыть все, показать только активную
    block.style.display = (i === locationIndex) ? 'block' : 'none';

    // Обновить next-image
    const nextImg = block.querySelector('.next-img');
    if (nextImg) {
      const nextIdx = (i + 1) % locationOrder.length;
      const nextId = locationOrder[nextIdx];
...
// Инициализация: показываем первую локацию
showLocation(0);

// ================== QUOTES ==================
document.addEventListener('DOMContentLoaded', () => {
  const quoteItems = Array.from(document.querySelectorAll('.quote-item'));
  const btn = document.querySelector('.next-quotes-btn');
  if (!quoteItems.length || !btn) return;
  ...
  const creaturesNextBtn = document.getElementById('creatures-next-btn');
  if (cards.length && creaturesNextBtn) {
    const PAGE_SIZE = 6;
    ...
    creaturesNextBtn.addEventListener('click', () => {
